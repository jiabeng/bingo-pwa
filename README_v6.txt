...
## 啟動
- 本機：`python app.py`
- Gunicorn（建議）：`gunicorn app:app --preload --timeout 120 --workers 2 --bind 0.0.0.0:$PORT`

## 端點
- `GET /api/latest`  
  從 `TLCAPIWeB/Lottery/LatestBingoResult` 取得最新一期（含 20 顆 + 超級獎號）。
...
- `GET /debug/official-snapshot`  
  抓官網結果頁（SPA）並寫檔 `data/last_today.html`。
- `GET /debug/last-html-head`、`GET /debug/last-html-download`  
  檢視/下載 `data/last_today.html`。
