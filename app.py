...
- 補齊今天：暫走備援 pilio 列表頁（可用 ?source=pilio）
- Debug：官方結果頁（SPA）快照下載/預覽

啟動（本機）：
  python app.py

啟動（Gunicorn，建議）：
  gunicorn app:app --preload --timeout 120 --workers 2 --bind 0.0.0.0:$PORT

注意：僅供資料彙整與研究，開獎資訊以官方為準。
"""
import os
import re
import time
...
# 基本設定
# -----------------------------
BASE_URL = "https://api.taiwanlottery.com/TLCAPIWeB"  # 由官網 Nuxt config 公開的 baseURL
...
LAST_HTML = DATA_DIR / "last_today.html"
...
def root():
    return jsonify({
        ...
        "endpoints": {
            ...
            "/debug/official-snapshot": "抓取官網結果頁（SPA）→ last_today.html",
...
def api_fetch_today_full():
    ...
    try:
        ...
        # 去重 + 依期別排序（新→舊）
        unique = {row["period"]: row for row in results}
